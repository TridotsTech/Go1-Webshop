...
class ProductQuery:
    ...
    def query(self, attributes=None, fields=None, search_term=None, start=0, item_group=None,sort_by=None):
        """
        Args:
        ...
        # query results
        if attributes:
            result, count = self.query_items_with_attributes(attributes, start,sort_by)
        else:
            result, count = self.query_items(start=start,sort_by=sort_by)
    ...
    def query_items(self, start=0,sort_by=None ):
        ...
        frappe.log_error("sort_by",sort_by)
        count_items = frappe.db.get_all(
            ...
            order_by = sort_by if sort_by else 'web_item_name desc',
    ...
    def query_items_with_attributes(self, attributes, start=0,sort_by=None):
        ...
        items, count = self.query_items(start=start,sort_by=sort_by)
