...
try:
    from go1_webshop.go1_webshop.doctype.go1_webshop_theme.go1_webshop_theme import log_css_template, get_css_content, get_curnet_doc
except ImportError:
    pass
...
class BuilderPage(WebsiteGenerator):
	...
	def get_context(self, context):
		...
		css_content = ''
		try:
			webshop_settings = frappe.db.get_value("Go1 Webshop Settings", "Go1 Webshop Settings", "selected_theme")
			webshop_theme = get_curnet_doc(webshop_settings)
			if webshop_theme:
				css_content = get_css_content(webshop_settings)
    
		except requests.exceptions.HTTPError as e:
			frappe.log_error(message=e, title="Error fetching data")

		content, style, fonts = get_block_html(blocks)
		context.fonts = fonts
		context.content = content
		css_content_details = css_content
		frappe.log_error("css_content_details", css_content_details)
		context.style = style + f"<style>{css_content_details}</style>"
	...
	def get_page_data(self, args=None):
		...
		builder_server_script = frappe.db.get_single_value("Builder Settings","custom_server_script")
		frappe.log_error(title="_locals_locals",message=_locals)
		if builder_server_script:
			if is_safe_exec_enabled():
				frappe.log_error("Name", self.name)
				safe_exec(builder_server_script, None, _locals, script_filename=self.name)
			else:
				safer_exec(builder_server_script, None, _locals, script_filename=self.name)
			page_data.update(_locals["data"])
		if self.page_data_script:
			# frappe.log_error(title="_locals",message=_locals)
			if is_safe_exec_enabled():
...
def extend_with_component(block):
	if block.get("extendedFromComponent"):
		...
		if component and component.block:
